...
class ResCompany(models.Model):
    ...
    def _get_imp_cfdi_node(self, cfdi_etree, attribute='tfd:TimbreFiscalDigital[1]',
                          namespaces={'tfd': 'http://www.sat.gob.mx/TimbreFiscalDigital'}):
        ''' Helper to extract relevant data from CFDI 3.3 nodes.
        By default this method will retrieve tfd, Adjust parameters for other nodes
        :param cfdi_etree:  The cfdi etree object.
        ...
        with zipfile.ZipFile(io.BytesIO(base64.b64decode(content[0]))) as z:
            for attachment_name in z.namelist():
                with z.open(attachment_name) as att_xml:
                    ...
                    trasladado_node = self._get_imp_cfdi_node(cfdi_etree)
                    retenido_node = self._get_imp_cfdi_node(cfdi_etree)
                    ...
                    r_payment_method = str(tree.get("MetodoPago"))
                    r_payment_form = str(tree.get("FormaPago"))
                    r_moneda = str(tree.get("Moneda"))
                    r_TotalImpuestosRetenidos = retenido_node.get("TotalImpuestosRetenidos")
                    r_TotalImpuestosTrasladados = trasladado_node.get("TotalImpuestosTrasladados")
                    ...
                    vals = dict(
                        ...
                        serie_folio=r_folio,
                        payment_method=r_payment_method,
                        payment_form=r_payment_form,
                        moneda=r_moneda,
                        TotalImpuestosRetenidos=r_TotalImpuestosRetenidos,
                        TotalImpuestosTrasladados=r_TotalImpuestosTrasladados,
    ...
    def download_cfdi_invoices_web(self, start_date=False, end_Date=False):
        ...
        if invoice_content_receptor:
            ...
            for uuid,data in invoice_content_receptor.items():
                ...
                r_payment_form = str(tree.get("FormaPago")) 
                r_moneda = str(tree.get("Moneda"))
                r_TotalImpuestosRetenidos = tree.get("TotalImpuestosRetenidos")
                r_TotalImpuestosTrasladados = tree.get("TotalImpuestosTrasladados")
                cfdi_version = tree.get("Version",'4.0')
                if cfdi_version=='4.0':
