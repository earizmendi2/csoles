...
class HrEmployee(models.Model):
    ...
    badge_line_ids = fields.One2many(
        "employee.badge.line", "employee_id", string="Badge History"
    )

    reward = fields.Selection(
        [
            ("month_best", "Month Best"),
            ("month_worst", "Month Worst"),
            ("year_best", "Year Best"),
            ("year_worst", "Year Worst"),
        ],
        string="Reward",
    )
                ...
                if employee_id:
                    ...
                    if employee_id.badge_line_ids:
                        for badge_line in employee_id.badge_line_ids:
                            if (
                                badge_line.date
                                and badge_line.date.month == current_month
                            ):
                                total_month_task += 1
                    ...
                    if total_month_task > 0 and total_month_points > 0:
                        month_avarage_reward_point = (
                            total_month_points / total_month_task
                        )
                    if total_year_task > 0 and total_year_points > 0:
                        year_avarage_reward_point = total_year_points / total_year_task

                    reward_result.append(
                        {
                            "emplpyee": employee_id,
                            "month_reward_point": month_avarage_reward_point,
                            "year_reward_point": year_avarage_reward_point,
                        }
                    )
            if reward_result:
                ...
                for line in reward_result:
                    ...
                    if (
                        ...
                        and 0 < worst_month_emp_points
                        and 0 < line["month_reward_point"]
                    ):
                        worst_month_emp_points = line["month_reward_point"]
                    ...
                    if (
                        ...
                        and 0 < worst_year_emp_points
                        and 0 < line["year_reward_point"]
                    ):
                        worst_year_emp_points = line["year_reward_point"]
            ...
            for employee_id in employee_ids:
                if employee_id:
                    ...
                    if best_month_employee and best_month_employee.id == employee_id.id:
                        reward_dict = {
                            "reward": "month_best",
                            "reward_points": best_month_emp_points,
                            "reward_date": date.today(),
                        }
                        employee_id.write(reward_dict)
                        self.create_and_attach_employee_certificate(
                            employee_id, "Best of the Month"
                        )
                    elif best_year_employee and best_year_employee.id == employee_id.id:
                        reward_dict = {
                            "reward": "year_best",
                            "reward_points": best_year_emp_points,
                            "reward_date": date.today(),
                        }
                        employee_id.write(reward_dict)
                        self.create_and_attach_employee_certificate(
                            employee_id, "Best of the Year"
                        )
                    elif (
                        worst_month_employee
                        and worst_month_employee.id == employee_id.id
                    ):
                        employee_id.write({"reward": "month_worst"})
                    elif (
                        worst_year_employee and worst_year_employee.id == employee_id.id
                    ):
    ...
    def create_and_attach_employee_certificate(self, employee_id, reward):
        # report_id variable is use for full fill the parameters
        report_id = self.env.ref(
            "bsi_employee_badges_and_rewards.action_employee_certificate_report"
        )
        pdf = self.env.ref(
            "bsi_employee_badges_and_rewards.action_employee_certificate_report"
        )._render_qweb_pdf(report_id.id, employee_id.id)
