...
#    Ivan Legarda Ed
#
#    Copyright (C) 2023-TODAY Ivan Legarda
#    Author: Ivan Legarda 
#
        ...
        for rec in self:
            ...
            if rec.time_span == 'daily':
                daily_so = self.env['sale.order'].search(
                    [('invoice_status', '=', 'invoiced')]).filtered(
            ...
            for order in orders:
                if order.user_id.id == rec.sale_user_id.user_id.id:
                    rec.person_achieved_amt = \
                        rec.person_achieved_amt + order.amount_untaxed
                if order.team_id.id == rec.team_id.id:
                    rec.team_achieved_amt = \
                        rec.team_achieved_amt + order.amount_untaxed
